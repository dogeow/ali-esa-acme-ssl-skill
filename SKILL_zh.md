...
使用同目录脚本：`scripts/esa_acme_issue.py`
...
- `--lang` 选择输出语言（默认 `en`；可用语言从 `scripts/i18n/` 自动发现）
...
3. AccessKey 设置了 IP 白名单后报权限/签名错误
